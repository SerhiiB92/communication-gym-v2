...
export default function ChatPage() {
  ...
  // Гард від "запізнілих" результатів розпізнавання: recognition.stop() асинхронний,
  // і після нього ще може прилетіти onresult зі старим аудіо. Без цього гарду такий
  // результат дописувався б у вже очищене поле вводу після відправки повідомлення.
  ...
  useEffect(() => {
    // Голосовий набір через вбудований у браузер Web Speech API — безкоштовно, без
    // сторонніх сервісів. Працює у Chrome (десктоп). Якщо браузер не підтримує —
    // кнопка мікрофона просто не показується.
    ...
    recognition.onresult = (event: any) => {
      // Ігноруємо результати, що прилетіли вже після того, як ми вирішили зупинити
      // слухання (наприклад, одразу після відправки повідомлення) — inache вони
      // "оживляють" вже очищене поле вводу.
      ...
      if (finalText) {
        // Web Speech API для uk-UA не розставляє розділові знаки, тому текст без
        // цього перетворюється на суцільний потік слів. Кожен "фінальний" фрагмент
        // зазвичай відповідає одній паузі в мовленні — тож оформлюємо його як окреме
        // речення: з великої літери і крапкою в кінці попереднього фрагмента.
        let segment = finalText.trim();
        if (segment) {
          segment = segment.charAt(0).toUpperCase() + segment.slice(1);
          const prev = baseTextRef.current.trim();
          const prevWithStop = prev && !/[.!?…]$/.test(prev) ? `${prev}.` : prev;
          baseTextRef.current = prevWithStop ? `${prevWithStop} ${segment} ` : `${segment} `;
        }
      }
  ...
  function toggleListening() {
    ...
    if (isListening) {
      ...
      try {
        ...
        acceptResultsRef.current = false;
        // вже запущено або інша помилка старту — ігноруємо
      }
    }
  }
