  ...
  if (notFound) {
    ...
    return (
          <div className="container">
            ...
            {result.exampleDialogue && result.exampleDialogue.length > 0 && (
                    <div className="report-section">
                              <h3>Приклад вдалого діалогу в цій ситуації</h3>
                              <div className="example-dialogue">
                                {result.exampleDialogue.map((line, i) => (
                                    <div key={i} className={`bubble-row ${line.role === "manager" ? "user" : "assistant"}`}>
                                                    <div className={`bubble ${line.role === "manager" ? "user" : "assistant"}`}>{line.text}</div>
                                    </div>
                                  ))}
                              </div>
                    </div>
